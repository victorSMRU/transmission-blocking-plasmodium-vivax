...
beta.estimate = summary(mod)$coefficients[2,1]
beta.se = summary(mod)$coefficients[2,2]
beta.error = 1.96*beta.se + beta.estimate
beta.lower= beta.estimate - beta.error
beta.upper = beta.estimate + beta.error
p.value = round(log10(summary(mod)$coefficient[2,4]), 2)

temp = dat[dat$regimen == 2 & dat$treatment == 0, ]
p = sum(temp$positive)/length(temp$positive)

rr.estimate = round(odds_to_rr(p=p, or=exp(beta.estimate)), 2)
rr.lower = round(odds_to_rr(p=p, or=exp(beta.lower)), 2)
rr.upper = round(odds_to_rr(p=p, or=exp(beta.upper)), 4)
rr.ci = paste0(rr.estimate, ' [95%CI ', rr.upper, ' to ', rr.lower, '], log10[p-value] = ', p.value)
rr_as_oo = rr.ci

# Chloroquine

beta.estimate = summary(mod)$coefficients[3,1]
beta.se = summary(mod)$coefficients[3,2]
beta.error = 1.96*beta.se + beta.estimate
beta.lower= beta.estimate - beta.error
beta.upper = beta.estimate + beta.error
p.value = round(log10(summary(mod)$coefficient[3,4]), 2)
...
rr.estimate = round(odds_to_rr(p=p, or=exp(beta.estimate)), 2)
rr.lower = round(odds_to_rr(p=p, or=exp(beta.lower)), 2)
rr.upper = round(odds_to_rr(p=p, or=exp(beta.upper)), 2)
rr.ci = paste0(rr.estimate, ' [95%CI ', rr.upper, ' to ', rr.lower, '], log10[p-value] = ', p.value)
rr_cq_oo = rr.ci

# Methylene blue

beta.estimate = summary(mod)$coefficients[4,1]
beta.se = summary(mod)$coefficients[4,2]
beta.error = 1.96*beta.se + beta.estimate
beta.lower= beta.estimate - beta.error
beta.upper = beta.estimate + beta.error
p.value = round(log10(summary(mod)$coefficient[4,4]), 2)

temp = dat[dat$regimen == 2 & dat$treatment == 0, ]
p = sum(temp$positive)/length(temp$positive)

rr.estimate = round(odds_to_rr(p=p, or=exp(beta.estimate)), 3)
rr.lower = round(odds_to_rr(p=p, or=exp(beta.lower)), 2)
rr.upper = round(odds_to_rr(p=p, or=exp(beta.upper)), 4)
rr.ci = paste0(rr.estimate, ' [95%CI ', rr.upper, ' to ', rr.lower, '], log10[p-value] = ', p.value)
rr_mb_oo = rr.ci
...
rr.estimate = round(odds_to_rr(p=p, or=exp(beta.estimate)), 4)
rr.lower = round(odds_to_rr(p=p, or=exp(beta.lower)), 2)
rr.upper = round(odds_to_rr(p=p, or=exp(beta.upper)), 4)
rr.ci = paste0(rr.estimate, ' [95%CI ', rr.upper, ' to ', rr.lower, '], log10[p-value] = ', p.value)
rr_as_sp = rr.ci
...
rr.estimate = round(odds_to_rr(p=p, or=exp(beta.estimate)), 4)
rr.lower = round(odds_to_rr(p=p, or=exp(beta.lower)), 2)
rr.upper = round(odds_to_rr(p=p, or=exp(beta.upper)), 5)
rr.ci = paste0(rr.estimate, ' [95%CI ', rr.upper, ' to ', rr.lower, '], log10[p-value] = ', p.value)
rr_mb_sp = rr.ci
...
beta_artesunate_oocyst = as.character(round(1/exp(my_res$`beta_drug[1]`[1]),2))
beta_artesunate_oocyst_lower = as.character(round(1/exp(my_res$`beta_drug[1]`[2]),2))
beta_artesunate_oocyst_upper = as.character(round(1/exp(my_res$`beta_drug[1]`[3]),2))

## Chloroquine

beta_chloroquine_oocyst = as.character(round(1/exp(my_res$`beta_drug[2]`[1]),2))
beta_chloroquine_oocyst_lower = as.character(round(1/exp(my_res$`beta_drug[2]`[2]),2))
beta_chloroquine_oocyst_upper = as.character(round(1/exp(my_res$`beta_drug[2]`[3]),2))

## Methylene blue

beta_methylene_blue_oocyst = as.character(round(1/exp(my_res$`beta_drug[3]`[1]),2))
beta_methylene_blue_oocyst_lower = as.character(round(1/exp(my_res$`beta_drug[3]`[2]),2))
beta_methylene_blue_oocyst_upper = as.character(round(1/exp(my_res$`beta_drug[3]`[3]),2))
...
beta_artesunate_sporozoite = as.character(round(1/exp(my_res$`beta_drug[1]`[1]),2))
beta_artesunate_sporozoite_lower = as.character(round(1/exp(my_res$`beta_drug[1]`[2]),2))
beta_artesunate_sporozoite_upper = as.character(round(1/exp(my_res$`beta_drug[1]`[3]),2))

## Chloroquine

beta_chloroquine_sporozoite = as.character(round(1/exp(my_res$`beta_drug[2]`[1]),2))
beta_chloroquine_sporozoite_lower = as.character(round(1/exp(my_res$`beta_drug[2]`[2]),2))
beta_chloroquine_sporozoite_upper = as.character(round(1/exp(my_res$`beta_drug[2]`[3]),2))

## Methylene blue

beta_methylene_blue_sporozoite = as.character(round(1/exp(my_res$`beta_drug[3]`[1]),2))
beta_methylene_blue_sporozoite_lower = as.character(round(1/exp(my_res$`beta_drug[3]`[2]),2))
beta_methylene_blue_sporozoite_upper = as.character(round(1/exp(my_res$`beta_drug[3]`[3]),2))
